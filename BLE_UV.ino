...
uint16_t uv_value = 0x10;
uint16_t ir_value = 0x10;
uint16_t hi_value = 0x10;
...
{
  uv_value = uv_dev.readUV();
  float UVindex = (float)uv_value;
  UVindex /= 100.0;
  #ifdef DEBUG
...
{
  hi_value = uv_dev.readVisible();
  #ifdef DEBUG
  Serial.print("Vis:"); Serial.print(hi_value);
  #endif    
  if (!connected) return;
  m_hi_handle();
}
